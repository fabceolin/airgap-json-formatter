...
- [ ] Task 4: Security hardening (AC: 5, 10)
  - [ ] Disable link navigation (prevent leaving app)
  - [ ] Block external resource loading
  - [ ] Replace external images with placeholder
  - [ ] Implement defense-in-depth sanitization check (verify no `<script>`, `<iframe>`, `on*` handlers, `javascript:` URLs)
  - [ ] Log warning and reject HTML if dangerous content detected (failsafe for Bridge Layer bypass)
  - [ ] Configure WebEngineView CSP: `script-src 'none'; object-src 'none'`
...
- [ ] Task 7: Scrolling and performance (AC: 8, 9, 11)
  - [ ] Implement smooth scrolling for large documents
  - [ ] Debounce preview updates (300ms)
  - [ ] Test with 500KB documents - MUST render within 200ms
  - [ ] Add performance timing instrumentation for render time measurement
  - [ ] Optimize if needed (consider virtual scrolling for >500KB)
...
- [ ] Performance verified: 500KB renders <200ms (AC11)
- [ ] Defense-in-depth sanitization check implemented (AC10)
- [ ] XSS test suite passes (OWASP payloads)
