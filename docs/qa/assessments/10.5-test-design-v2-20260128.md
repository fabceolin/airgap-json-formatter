# Test Design: Story 10.5 - Markdown Preview Pane (v2)

**Date:** 2026-01-28
**Designer:** Quinn (Test Architect)
**Story:** 10.5 - Markdown Preview Pane
**Story Version:** v1.3 (includes AC10 defense-in-depth, AC11 performance SLA)
...
- **Unit tests:** 9 (24%)
- **Integration tests:** 19 (50%)
- **E2E tests:** 10 (26%)
- **Priority distribution:** P0: 16, P1: 13, P2: 7, P3: 2

### Strategy Rationale

This story has **critical security concerns** (XSS, external resource blocking) requiring extensive security-focused testing. AC10 adds explicit defense-in-depth sanitization requirements. AC11 adds measurable performance SLAs. The WebEngineView rendering approach adds complexity that shifts coverage toward integration tests. The security-critical nature elevates many scenarios to P0.
...
### AC10: Security Defense-in-Depth (NEW in v1.2)

| ID | Level | Priority | Test | Justification | Mitigates |
|----|-------|----------|------|---------------|-----------|
| 10.5-SEC-007 | Integration | P0 | HTML containing `<script>` tags is rejected with warning log | Defense-in-depth failsafe | SEC-001 |
| 10.5-SEC-008 | Integration | P0 | HTML containing `<iframe>` tags is rejected with warning log | Defense-in-depth failsafe | SEC-001 |
| 10.5-SEC-009 | Integration | P0 | HTML containing `on*` event handlers is rejected | Defense-in-depth failsafe | SEC-001 |
| 10.5-SEC-010 | Integration | P0 | HTML containing `javascript:` URLs is rejected | Defense-in-depth failsafe | SEC-001 |

### AC11: Performance SLA - 200ms/500KB (NEW in v1.2)

| ID | Level | Priority | Test | Justification | Mitigates |
|----|-------|----------|------|---------------|-----------|
| 10.5-PERF-001 | Integration | P0 | 500KB markdown document renders within 200ms | Performance SLA validation | PERF-001 |
| 10.5-PERF-002 | E2E | P1 | Performance timing instrumentation reports accurate render time | Monitoring capability | PERF-001 |
...
| 10.5-SEC-007 | Integration | P0 | Defense-in-depth: reject HTML with script tags | Failsafe validation | SEC-001 |
| 10.5-SEC-008 | Integration | P0 | Defense-in-depth: reject HTML with iframe tags | Failsafe validation | SEC-001 |
| 10.5-SEC-009 | Integration | P0 | Defense-in-depth: reject HTML with on* handlers | Failsafe validation | SEC-001 |
| 10.5-SEC-010 | Integration | P0 | Defense-in-depth: reject HTML with javascript: URLs | Failsafe validation | SEC-001 |

## Risk Coverage Matrix

| Risk ID | Description | Test Coverage |
|---------|-------------|---------------|
| SEC-001 | XSS via malicious HTML/SVG | 10.5-SEC-001 to 10.5-SEC-010 (10 tests) |
...
| PERF-001 | Large document rendering | 10.5-UNIT-008, 10.5-E2E-006, 10.5-PERF-001, 10.5-PERF-002 (4 tests) |
...
| large.md | 500KB | Generated content with tables | Performance testing (AC11) |
| malicious.md | 5KB | OWASP XSS payloads | Security testing (AC10) |
...
- Memory profiler
- Render time instrumentation (for AC11 200ms validation)
- High-resolution timer for sub-200ms measurements
...
1. 10.5-SEC-001 through 10.5-SEC-010 (XSS prevention + defense-in-depth)
2. 10.5-UNIT-005, 10.5-INT-011, 10.5-E2E-004 (External resource blocking)
3. 10.5-INT-008 (Link navigation blocking)

### Phase 2: Performance SLA (P0)
4. 10.5-PERF-001 (200ms/500KB validation)
5. 10.5-PERF-002 (Performance instrumentation)

### Phase 3: Core Functionality (P1)
6. 10.5-UNIT-001 through 10.5-UNIT-004 (Core logic)
7. 10.5-INT-001 through 10.5-INT-010 (Component integration)
8. 10.5-E2E-001, 10.5-E2E-002 (User journeys)

### Phase 4: Secondary Features (P2)
9. Remaining unit tests
10. Copy functionality tests
11. Theme switching E2E

### Phase 5: Nice-to-Have (P3)
12. Rapid typing debounce optimization test
...
test_design:
  ...
  by_level:
    unit: 9
    integration: 19  # Including 10 SEC tests
    e2e: 10
  by_priority:
    p0: 16  # Security + Performance SLA
    p1: 13
    p2: 7
    p3: 2
  coverage_gaps:
  ...
  security_tests:
    ...
    link_navigation: 1
  performance_tests:
    sla_validation: 1  # AC11: 200ms/500KB
...
Test design matrix: docs/qa/assessments/10.5-test-design-v2-20260128.md
P0 tests identified: 16
Security tests (SEC-001 mitigation): 10 (6 prevention + 4 defense-in-depth)
Performance SLA tests (AC11): 2
